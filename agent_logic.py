    ...
    for line in pattern_scan.get("stdout", "").splitlines():
        if "test" not in line and "__pycache__" not in line:
            if "yaml.load(" in line:
                auto_fixable.append(line[:200])
            else:
                manual_review.append(line[:200])
    if auto_fixable:
        findings.append({"type": "auto_fixable", "count": len(auto_fixable), "matches": auto_fixable[:10],
                         "fix": "Replace yaml.load() with yaml.safe_load()"})
    if manual_review:
        findings.append({"type": "needs_review", "count": len(manual_review), "matches": manual_review[:10],
                         "note": "Requires human judgment — may be legitimate or RCE risk"})
...
def handle_security_fix(payload: dict) -> dict:
    path = payload.get("path", ".")
    commit_message = payload.get("message", "fix: auto-fix security issues")
    token = _gh_token()
    if not token:
        return {"error": "GITHUB_TOKEN not available"}
    results = []
    ...
    yaml_fix = run([
        "sed", "-i", "", "-E",
        "s/yaml\\.load\\(/yaml.safe_load(/g",
        "--", path
    ] if path != "." else [
        "bash", "-c",
        "find . -name '*.py' -not -path './.*' -exec sed -i '' -E 's/yaml\\.load\\(/yaml.safe_load(/g' {} +"
    ], timeout=30)
    if yaml_fix["exit_code"] == 0:
        results.append({"fix": "yaml.load → yaml.safe_load", "applied": True})
    # Run ruff security rules
    ruff = run(["python", "-m", "ruff", "check", "--fix", "--select", "S", "--quiet", path], timeout=60)
    results.append({"fix": "ruff security rules (S)", "exit_code": ruff["exit_code"],
                    "remaining": ruff["stdout"][:500] or "none"})
    diff = run(["git", "diff", "--stat"])
    has_changes = bool(diff.get("stdout"))
    if has_changes:
        run(["git", "config", "user.name", "A2A Agent"])
        ...
        c = run(["git", "commit", "-m", commit_message, "--allow-empty"])
        run(["git", "push"], timeout=30)
        results.append({"commit": c.get("stdout","")[:100]})
    return {"fixes_applied": len(results), "results": results, "has_changes": has_changes,
            "unfixed": "Run /g scan to review remaining issues that need manual attention"}
...
def handle_help(_) -> dict:
    return {
        "tasks": {
            ...
            "security_fix": "Auto-fix yaml.load → safe_load and ruff security rules",
...
TASK_HANDLERS = {
    ...
    "security_fix": handle_security_fix,
    "help": handle_help,
}
